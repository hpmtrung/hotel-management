...
describe('JhiItemCountComponent test', () => {
  describe('UI logic tests', () => {
    it('should change the content on page change', () => {
      const { rerender } = render(<ItemCountComponent page={1} itemsPerPage={10} total={100} />);

      expect(screen.getByText('Showing 1 - 10 of 100 items.')).not.toBeNull();

      rerender(<ItemCountComponent page={2} itemsPerPage={10} total={100} />);
  ...
  describe('Translation tests', () => {
    it('should change on language change', () => {
      ...
      const mountedWrapper = render(<ItemCountComponent page={1} itemsPerPage={10} total={100} i18nEnabled />);
      expect(mountedWrapper.getByText('Showing 1 - 10 of 100 items.')).not.toBeNull();

      TranslatorContext.setLocale('fr');
      const mountedWrapperFr = render(<ItemCountComponent page={1} itemsPerPage={10} total={100} i18nEnabled />);
